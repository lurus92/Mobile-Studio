"use strict";
var os_1 = require("os");
var AndroidDebugBridgeResultHandler = (function () {
    function AndroidDebugBridgeResultHandler($logger, $errors) {
        this.$logger = $logger;
        this.$errors = $errors;
    }
    AndroidDebugBridgeResultHandler.prototype.checkForErrors = function (adbResult) {
        var errors = [];
        if (adbResult) {
            if (_.isArray(adbResult)) {
                _.each(AndroidDebugBridgeResultHandler.ANDROID_DEBUG_BRIDGE_ERRORS, function (error) {
    ...
    AndroidDebugBridgeResultHandler.prototype.handleErrors = function (errors, treatErrorsAsWarnings) {
        var _this = this;
        _.each(errors, function (error) {
            _this.$logger.trace("Error name: " + error.name + " result code: " + error.resultCode);
        });
        var errorMessages = _(errors).map(function (error) { return error.description; })
            .join(os_1.EOL)
            .toString();
    ...
    return AndroidDebugBridgeResultHandler;
}());
